...
class FileWriter {
    ...
    writeBaseType(schema) {
        ...
        this.appendLine(`func New${schema.title}(${ref} ${schema.title}) *${schema.title} {`)
        this.appendLinesWithTabs(1, `return &${ref}`)
        this.appendLine(`}`)
        ...
        if (schema.enum) {
            ...
            this.appendLine(')')
            this.appendLine(`var All${schema.title} = []${schema.title}{`)
            schema.enum.forEach(e => {
                this.appendLinesWithTabs(1, `${schema.title}${formatEnumName(e)},`)
            })
            this.appendLine(`}`)
    ...
    writeStruct(schema, name, argRef) {
        const ref = argRef || (name || schema.title).charAt(0).toLowerCase()
        ...
        Object.keys(schema.properties).forEach((p) => {
            ...
            const required = schema.required?.length ? property.type === 'array' || property.goInterface || schema.required.some(r => r === p) : property.type === 'array' || property.goInterface
        ...
        if (!schema.goNoIsValid) {
            ...
            this.appendLinesWithTabs(1, `}`)
            if (schema.goImplements) {
                this.appendLinesWithTabs(1, `if !${ref}.Base${schema.goImplements}.IsValid() {`)
                this.appendLinesWithTabs(2, `return false`)
                this.appendLinesWithTabs(1, `}`)
            }
            Object.keys(schema.properties).forEach((p) => {
    ...
    writeInterface(schema) {
        ...
        this.appendLinesWithTabs(1, `IsValid() bool`)
        Object.keys(schema.properties).forEach((p) => {
            const property = schema.properties[p]
        ...
        if (!schema.noGoBase) {
            ...
            this.writeStruct({...schema}, `Base${schema.title}`, ref)
            Object.keys(schema.properties).forEach((p) => {
                const property = schema.properties[p]
                const required = schema.required?.length ? schema.required.some(r => r === p) : false
